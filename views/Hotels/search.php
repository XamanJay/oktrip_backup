...
<body id="search">
	...
	<script>
	 ...
	 /* La linea inferior es Código que debe estar comentado

		var hoteles = <?php//  echo json_encode($hoteles); ?> ; */ 

		

	var hoteless = <?php  echo json_encode($hoteles); ?> ;  

	var hoteles = hoteless.filter(xxFunction);

	

	function xxFunction(Hotel) {

	        //return Hotel.Rooms[0].MealPlans[0].AgencyPublic.AgencyPublic > 4100.00;  

		    //return Hotel.Rooms[0].MealPlans[0].AgencyPublic.AgencyPublic > 2500.00;  <--- menor a 600

			return Hotel.Rooms[0].MealPlans[0].AgencyPublic.AgencyPublic > 5000.00;  

		   }

	document.getElementById("demo").innerHTML = hoteles.length;	

	//console.log (hoteles) ;

	//Código para filtrar habitaciones de con precio mayor a 600 o 700 pesos
...
function printHotel(hotel)
{
	...
	var desc = (precioClub != 0) ? parseInt((priceClub*100)/priceNormal) : parseInt((priceOkPerNight*100)/priceNormal);
	var etiqueta = "";
	var bestOffer = "";
	if(desc >= 30)
		etiqueta = "<img src='/img/iconos/sale.jpg' alt='Sale' style='width:45px; margin-left:15px;margin-top:-10px;' data-toggle='tooltip' title='Hasta "+desc+"% en reserva de habitacion'>";

	if(desc >= 45 )
		bestOffer = "<img src='/img/iconos/sales.png' alt='Sale' style='width:65px;position:absolute;margin-top:-5px;margin-left:-10px;' data-toggle='tooltip' title='Hasta "+desc+"% en reserva de habitacion'>";

	url = "/hoteles/details/"+lang+"?idDestiny="+idCity+"&idHotel="+hotel.Id + _url;

	print = "<div class='item-hotel'>"+
	"<div class='row'>"+
	"<div class='col-sm-3'>"+bestOffer+
	...
	"<a class='cl-ok style-none' href='#' onclick='initMapHotel("+hotel.Latitude+","+hotel.Longitude+", \""+hotel.Name+"\");'>"+hotel.CityName+"<i class='fa fa-map-marker' aria-hidden='true'></i></a>"+etiqueta+
	...
	if(parseFloat(priceNormal) > parseFloat(priceOkPerNight) )   {

	print +="<div class='cl-ok'>"+currency+" <span class='price'>$"+parseFloat(priceOkPerNight).toLocaleString("<?php echo $GLOBALS['Locale_string']; ?>")+"</span></div>";
	...
	else if (parseFloat(priceOkPerNight) > parseFloat(priceNormal)) {
	print +="<div class='cl-ok'>"+currency+" <span class='price'><strike>$"+parseFloat(priceOkPerNight).toLocaleString("<?php echo $GLOBALS['Locale_string']; ?>")+"</strike></span></div>";
	print +="<div class='cl-ok'>"+currency+" <span class='price2'>$"+parseFloat(priceNormal).toLocaleString("<?php echo $GLOBALS['Locale_string']; ?>")+"</span></div>";	
	}
	else if (parseFloat(priceOkPerNight) == parseFloat(priceNormal)) {
	print +="<div class='cl-ok'>"+currency+" <span class='price'>$"+parseFloat(priceOkPerNight).toLocaleString("<?php echo $GLOBALS['Locale_string']; ?>")+"</strike></span></div>";
	print +="<div class='cl-ok'>"+currency+" <span class='price2'><strike>$"+parseFloat(priceNormal).toLocaleString("<?php echo $GLOBALS['Locale_string']; ?>")+"</strike></span></div>";	
