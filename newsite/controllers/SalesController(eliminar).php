...
public function getTourscontenedor(){
	...
	$query = "SELECT Id_xtours, titulo, semblanza, Id_xtipotours, Id_xcategorias, Id_xproveedores, Id_xlocalidades,  path_imagen, descripcion, incluye, informacion_adicional, precio_pesos, description, includes,
                                  additional_info, price_dollars, xcategorias, xlocalidades, xproveedores, xtipotours
			         FROM VISTA__TOURS
					 WHERE isdeleted = 0 ;   "	;
...
try
	{
	   ...
	   $precio_adulto_pesos		= $_POST["precio_adulto_pesos"];		if (empty($precio_adulto_pesos)) { $precio_adulto_pesos = '---' ;  } else { $precio_adulto_pesos = $_POST['precio_adulto_pesos'] ; }     
	   $precio_infante_pesos		= $_POST["precio_infante_pesos"];	if (empty($precio_infante_pesos)) { $precio_infante_pesos = '---' ;  } else { $precio_infante_pesos = $_POST['precio_infante_pesos'] ; }     
	   ...
	   $adult_price_dollars			= $_POST["adult_price_dollars"];		if (empty($adult_price_dollars)) { $adult_price_dollars = '---' ;  } else { $adult_price_dollars = $_POST['adult_price_dollars'] ; }     
	   $infants_price_dollars		= $_POST["infants_price_dollars"];		if (empty($infants_price_dollars)) { $infants_price_dollars = '---' ;  } else { $infants_price_dollars = $_POST['infants_price_dollars'] ; }     
...
$db = new db();
$conn = $db->conn_local();
$query2 =" INSERT INTO volaris ( titulo, semblanza, descripcion, itinerario, incluye, no_incluye, precio_adulto_pesos, precio_infante_pesos, sugerencias, informacion_adicional, transportacion, 
