...
$Oct_1 = $eterno.'-10-01'; $Oct_2 = $eterno.'-10-31';  $Nov_1 = $eterno.'-11-01'; $Nov_2 = $eterno.'-11-30'; $Dic_1 = $eterno.'-12-01'; $Dic_2 = $eterno.'-12-31';
		...
		$GRAN_TOT_VTAS_WEB = $NETO_0000_VTAS_WEB + $PUBLICO_0000_VTAS_WEB;
		 ...
		 /*-----------------------------*/    	
			...
			$query_316_0000 =" SELECT SUM(total_neto) 'NETO_0000_VTAS_TRAS_12USD',   SUM(total_publico) 'PUBLICO_0000_VTAS_TRAS_12USD',  SUM(paxxx) 'TOT_PAX_12USD_0000'
					           FROM volaris
					           WHERE Id_productos = 316
						        AND isDeleted = 0
						        AND fecha_llegada BETWEEN CAST('".$eterno."-01-01' AS DATE)
						        AND CAST('".$eterno."-12-31' AS DATE); "; 
	       // print  $query_316_0000 ; 
 ...
 /*-----------------------------*/
            ...
            $query_235_0000 =" SELECT SUM(total_neto) 'NETO_0000_VTAS_TRAS_PRIVADA', 
			                          SUM(total_publico) 'PUBLICO_0000_VTAS_TRAS_PRIVADA',  
									  SUM(paxxx) 'TOT_PAX_PRIVADA_0000'
			        		      FROM volaris
					              WHERE Id_productos = 235
						          AND isDeleted = 0
						          AND fecha_llegada BETWEEN CAST('".$eterno."-01-01' AS DATE)
						          AND CAST('".$eterno."-12-31' AS DATE); "; 
			 print  $query_235_0000 ; 				 
       		$stmt_235_0000 = $conn->prepare($query_235_0000);
			$stmt_235_0000->execute();
        	$row = $stmt_235_0000->fetch(PDO::FETCH_ASSOC);
			$NETO_0000_VTAS_TRAS_PRIVADA = $row["NETO_0000_VTAS_TRAS_PRIVADA"]; if (empty($NETO_0000_VTAS_TRAS_PRIVADA)) { $NETO_0000_VTAS_TRAS_PRIVADA =   0 ;  } 
            $PUBLICO_0000_VTAS_TRAS_PRIVADA = $row["PUBLICO_0000_VTAS_TRAS_PRIVADA"]; if (empty($PUBLICO_0000_VTAS_TRAS_PRIVADA)) { $PUBLICO_0000_VTAS_TRAS_PRIVADA =  0 ;  } 
		  $GRAN_TOT_VTAS_TRAS_PRIVADAS = $NETO_0000_VTAS_TRAS_PRIVADA + $PUBLICO_0000_VTAS_TRAS_PRIVADA ;
            $TOT_PAX_PRIVADA_0000 = $row["TOT_PAX_PRIVADA_0000"]; if (empty($TOT_PAX_PRIVADA_0000)) { $TOT_PAX_PRIVADA_0000 = 0 ;  } 
...
$query2 =" INSERT INTO xtours ( titulo, semblanza, descripcion, itinerario, incluye, no_incluye, precio_adulto_pesos, precio_infante_pesos, sugerencias, informacion_adicional, transportacion, 
                                                     observaciones, Id_xtipotours, Id_xcategorias, Id_xproveedores, Id_xlocalidades, path_imagen, title, semblance, description, itinerary, includes, without_include,
...
try {
 ...
 $conn = $db->conn_local();
	 ...
	 $query2 = "SELECT  Id_xtours, xtours, titulo, semblanza, descripcion, itinerario, incluye, no_incluye, precio_adulto_pesos, precio_infante_pesos, sugerencias, informacion_adicional,
                                       transportacion, observaciones, path_imagen, duracion, disponible, precio_pesos, title, semblance, description, itinerary, includes, without_include, adult_price_dollars,
...
$query2 = " UPDATE xtours
                   SET titulo = '$titulo',  semblanza = '$semblanza',  descripcion = '$descripcion',  itinerario = '$itinerario',  incluye = '$incluye',   no_incluye = '$no_incluye', precio_adulto_pesos = '$precio_adulto_pesos',
						   precio_infante_pesos = '$precio_infante_pesos',   sugerencias = '$sugerencias',  informacion_adicional = '$informacion_adicional',  transportacion = '$transportacion',
